...
int Fbonded_eval_dihed(Fbonded *p, const dvec *pos, dvec *f, Energy *e) {
  const DihedPrm *dihedprm = ForcePrm_dihedprm_array(p->fprm);
  const Dihed *dihed = Topology_dihed_array(p->topo);
  int32 ndiheds = Topology_dihed_array_length(p->topo);
  int32 n, i, j, k, l, pid;
  int s;

  for (n = 0;  n < ndiheds;  n++) {
    pid = dihed[n].dihedPrmID;
    if (FAIL == pid) continue;
    i = dihed[n].atomID[0];
    j = dihed[n].atomID[1];
    k = dihed[n].atomID[2];
    l = dihed[n].atomID[3];
    if ((s=Fbonded_eval_dihed_term(p, &dihedprm[pid],
...
int Fbonded_eval_dihed_term(Fbonded *p,
    const DihedPrm *prm,
    const dvec *pos_i,
    const dvec *pos_j,
    const dvec *pos_k,
  ...
  const DihedTerm *dterm = DihedPrm_term_array(prm);
  const int32 ndterms = DihedPrm_term_array_length(prm);
  int32 i;

  if (0 == ndterms) return OK;  /* nothing to evaluate! */
  ...
  K = K1 = 0;
  for (i = 0;  i < ndterms;  i++) {
    dreal k = dterm[i].k_dihed;
    dreal delta = dterm[i].phi0;
    int32 n = dterm[i].n;
    if (n) {

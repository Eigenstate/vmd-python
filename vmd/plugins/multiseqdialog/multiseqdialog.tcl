...
package provide multiseqdialog 1.1
...
#  Required - is this download required for proper operation of MultiSeq?  Y/N
#  envVariables - additional variables. A list of name/value pairs
                ...
                if {[array size options] > 0 && $options(update) == 1} {
                    ...
                    foreach update $updateList {
                        ...
                        if {[string first "couldn't open socket" $errorMessage] != -1 ||   \
                                               [string first "is not available" $errorMessage] != -1 } {
           ...
           puts "Supposedly got version data, but it was empty!"
        }
    }
}
...
proc ::MultiSeqDialog::showPreferencesDialog {message arg_changeNotifier} {
    ...
    set helpURL [string trimright [vmdinfo www]]plugins/multiseq
    ...
    if {$metadataDir == ""} {
        ...
        -type ok -message \
        "Before using MultiSeq, you must select a directory in which to store metadata. If you choose \"Cancel\", a default directory will be used.  Additionally, you will need to have a working network connection to download the necessary metadata files." 
...
proc ::MultiSeqDialog::readInfoFile { } {
    ...
    # Try to get either platform or generic version of info file from the web.
    set data ""
    ...
    foreach location [list $platformInfoURL $infoURL $backupPlatformInfoURL $backupInfoURL $platformInfoFile $infoFile] {
        ...
        if {[catch {
            puts "MultiSeq Info) Retrieving multiseq db version from: $location"
            if {[string first "http" $location] == 0} {
                vmdhttpcopy $location $tempFileName
                if {[file exists $tempFileName]} {
                   if {[file size $tempFileName] > 0} {
                      ...
                      puts "MultiSeq Error) message: 0 bytes retrieved"
                   }
                } else {
                   puts "MultiSeq Error) message: Failed to retrieve version file."
                }
            } else {
    ...
    if {[file exists $fullFilename] > 0} {
      if {[file size $fullFilename] > 0} {
        ...
        setFilename $key $filename    
         ...
         return -code error "$fullFilename is not available.  0 bytes"
      }
    } else {
      return -code error "$fullFilename is not available."
    }
}

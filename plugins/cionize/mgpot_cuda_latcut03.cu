...
#ifdef CPARM
__constant__ int srad;
__constant__ int padding;
#endif
    ...
    )
{
...
#ifdef SHIFT
  float *qlevel = qgrids + (soff<<LG_SUBCUBESZ);
#else
  ...
  int boff = blockIdx.x - (level > 0 ? sinfo[level-1].w : 0);
    /* threadblock offset */
#ifdef SHIFT
  int sbx = nx - (padding<<1);  /* dimensions of inner subcube array */
  int sby = ny - (padding<<1);
#else
...
#ifdef SHIFT
              float *q = qlevel + (qsid<<LG_SUBCUBESZ);
              int idest = ((((tz+(k<<2))<<3) + (ty+(j<<2)))<<3) + (tx+(i<<2));
#else
              float *q = qlevel + qsid * SUBCUBESZ;
...
#ifdef SHIFT
              int cq_index = ((((tz+k)<<3) + (ty+j))<<3) + (tx+i);
#ifdef WTRAD
              int wt_index = ((wz+k)*(wtrad) + (wy+j))*(wtrad) + (wx+i);
#else
              int wt_index = ((wz+k)*(srad<<3) + (wy+j))*(srad<<3) + (wx+i);
#endif /* WTRAD */
#else
...
#ifdef SHIFT
  float *eout = egrids + ((soff + (sz*ny + sy)*nx + sx)<<LG_SUBCUBESZ);
#else
...
extern "C" int mgpot_cuda_latcut03(Mgpot *mg) {
  const int nlevels = mg->lk_nlevels;
  const int wt_total = (8*mg->lk_srad) * (8*mg->lk_srad) * (8*mg->lk_srad);
...
#ifdef CPARM
  cudaMemcpyToSymbol(srad, &(mg->lk_srad), sizeof(int), 0);
  CUERR;
  cudaMemcpyToSymbol(padding, &(mg->lk_padding), sizeof(int), 0);
  CUERR;
#endif
  ...
  /* invoke kernel */
  latcut_kernel<<<gridDim, blockDim, 0>>>(
#if !defined(CPARM)
      mg->lk_srad,
      mg->lk_padding,
#endif

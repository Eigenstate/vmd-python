...
def test_read(file_3frames):
    ...
    with pytest.warns(DeprecationWarning):
        molecule.read(m, "pdb", filename=file_3frames, beg=1, end=1, skip=1, waitfor=-1)
    assert molecule.numframes(m) == 4
...
def test_write(tmpdir, file_3frames):
    ...
    with pytest.warns(DeprecationWarning):
        molecule.write(
            m, "psf", beg=0, end=1, filename=os.path.join(tmpdir, "deprecated.psf")
        )
    ...
    with pytest.raises(ValueError):
        molecule.write(m, "mae", os.path.join(tmpdir, "badsel.mae"), selection=sel2)
...
def test_mol_attrs(file_3nob):
    ...
    molecule.set_periodic(
        m2, frame=0, a=90.0, b=90.0, c=90.0, alpha=90.0, beta=90.0, gamma=90.0
    )
    assert (
        list(molecule.get_periodic(m2, frame=0).values()) == [pytest.approx(90.0)] * 6
    )
...
def test_mol_descstrs(file_3frames):
    ...
    # Remarks
    assert molecule.get_remarks(molid=m1) == [
        "REMARK 1   remark 1\nREMARK 2" "   also remark 2\n"
    ]
...
def test_volumetric(file_psf, file_dx):
    ...
    with pytest.warns(DeprecationWarning):
        molecule.add_volumetric(
            molid=m, name="testvol", xsize=3, ysize=6, zsize=10, data=newvol
        )
    assert molecule.num_volumetric(m) == 1
